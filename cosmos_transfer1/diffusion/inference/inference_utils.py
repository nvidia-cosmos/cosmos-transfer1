...
def get_ctrl_batch(
    model, data_batch, num_video_frames, input_video_path, control_inputs, blur_strength, canny_threshold, cutoff_frame, input_video_tensor=None
):
    """Prepare complete input batch for video generation including latent dimensions.
    ...
    # ------------------------------------------------------------
    # Handle the RGB context video (either provided as tensor or path)
    # ------------------------------------------------------------
    if input_video_tensor is not None or input_video_path:
        ...
        # --------------------------------------------------------
        # Apply cutoff_frame logic (freeze first N frames)
        # --------------------------------------------------------
        if cutoff_frame != -1 and cutoff_frame > 0:
            ...
            guided_mask[:, :, :latent_cutoff_frame] = 1  # same spatial dims broadcast
    ...
    for hint_key, control_info in control_inputs.items():
        if "input_control" in control_info:
            ...
            # --------------------------------------------------------
            # NEW:  accept in-memory numpy arrays to bypass disk I/O.
            # --------------------------------------------------------
    ...
    if input_video_path or input_video_tensor is not None:
        if input_video_path:
            print(f"input_video_path: {input_video_path}")
        # Do NOT trim the video; it already has the required `num_video_frames` length after padding.
