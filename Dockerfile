...
# Install basic tools
RUN apt-get update && apt-get install -y git tree ffmpeg
...
# Cosmos User Setup: Disable these lines if you want to run as root (not recommended)
ARG USERNAME=cosmos
ARG USER_UID=2305
ARG USER_GID=2305
RUN groupadd -g $USER_GID $USERNAME && useradd -m -u $USER_UID -g $USERNAME $USERNAME
USER $USERNAME
WORKDIR /home/$USERNAME
# Cosmos User Setup: End of user setup
